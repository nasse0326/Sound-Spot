 ...
 * Seamlessly resolves login-required studios using session cookies.
 */
import fs from 'fs';
...
/**
 * Fetches 21 days of availability slots for Noah Akihabara rooms using pure Node fetch.
 */
export async function fetchNoahAkibaDays(
  baseDate: Date = new Date(),
  dayCount: number = 21
): Promise<NoahRoomData[]> {
  console.log(`📡 [NOAH Akiba] サウンドスタジオノア秋葉原店のリアル空き枠を取得中 (Node fetch / ${dayCount}日間)...`);
  ...
  for (const st of NOAH_AKIBA_ROOM_MAP) {
    rawData[st.studioId] = [];
      ...
      try {
        ...
        const res = await fetch(url, {
          headers: {
            ...
            'Referer': 'https://www.studionoah.jp/noahweb/webs/chart/akihabara/',
