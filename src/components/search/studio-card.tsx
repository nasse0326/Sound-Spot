          ...
          if (compact) {
            return (
              <div
                ...
                className="py-3 px-3 rounded-xl hover:bg-slate-800/50 active:bg-slate-800/80 transition-all flex items-center gap-3 cursor-pointer group"
              >
                ...
                <div className="flex items-center gap-2 min-w-0 flex-1">
                  ...
                  <span className="text-xs font-semibold text-slate-300 bg-slate-800/90 px-2 py-0.5 rounded border border-slate-700/60 shrink-0">
                    {room.sizeTatami}帖
                  </span>
                  ...
                  <span className="text-sm text-slate-400 truncate">
                    {ampTags.join(' / ')}
                    {room.hasRecording && (
                      <span className="text-purple-400 font-medium ml-2">REC</span>
                    )}
                  </span>
                </div>
                ...
                <div className="flex items-center gap-3 shrink-0">
                  ...
                  {availResult.matchType === 'exact' ? (
                    <span className="px-2.5 py-1 rounded-md text-xs font-bold bg-emerald-950 text-emerald-300 border border-emerald-600/80 flex items-center gap-1">
                      <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" />
                      <span>空き</span>
                    </span>
                  ) : availResult.matchType === 'early30' || availResult.matchType === 'late30' ? (
                    <span className="px-2.5 py-1 rounded-md text-xs font-bold bg-blue-950 text-blue-300 border border-blue-500/80 flex items-center gap-1">
                    ...
                    <span className="px-2.5 py-1 rounded-md text-xs font-medium bg-slate-900 text-slate-400 border border-dashed border-slate-700/80">
                      未取得
                    </span>
                  ) : (
                    <span className="px-2.5 py-1 rounded-md text-xs font-medium bg-slate-950 text-slate-500 border border-slate-800 flex items-center gap-1">
